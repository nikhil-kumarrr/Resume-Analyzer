...
:root {
    ...
    --grad3:      linear-gradient(135deg, #0ea5e9 0%, #10b981 100%);
    --sh1:        0 1px 3px #2563eb0f, 0 4px 16px #2563eb08;
    --sh2:        0 4px 24px #2563eb14, 0 1px 4px #0a0e2e08;
    --sh3:        0 8px 40px #2563eb18, 0 2px 8px #0a0e2e0c;
}
...
.nav-logo-mark {
    ...
    font-size: 1.05rem;
    box-shadow: 0 4px 12px #2563eb2a;
}
.nav-logo-text {
...
.nbadge-active {
    border-color: #2563eb30; color: var(--blue);
...
.hero-h1 {
    ...
    line-height: 1.08; color: var(--text1); margin-bottom: 1rem;
}
.hero-h1 em {
    font-style: normal;
    background: var(--grad);
...
.res-panel-badge {
    font-family: 'JetBrains Mono', monospace;
    font-size: 0.56rem; color: var(--blue);
    background: #2563eb0a; border: 1px solid #2563eb20;
...
.rcard-best {
    background: linear-gradient(135deg, #eff6ff 0%, #f0feff 100%);
...
.empty-ico { font-size: 2.5rem; display: block; margin-bottom: 0.9rem; opacity: 0.15; }
...
PROG_COLORS = {
    1: ('linear-gradient(90deg,#2563eb,#0ea5e9)', '#2563eb'),
    2: ('linear-gradient(90deg,#7c3aed,#2563eb)', '#7c3aed'),
    3: ('linear-gradient(90deg,#0ea5e9,#10b981)', '#0ea5e9'),
}
...
with c_left:
    ...
    with tab1:
        resume_text = st.text_area(
            ...
            placeholder="Paste resume content here...\n\nTip: Include skills, experience &\neducation for best accuracy.",
            height=270, key="txt"
        )
    ...
    with tab2:
        ...
        if up:
            ...
            if pt.strip():
                ...
                st.markdown("""
                <div class="ib ib-err">
                  ✕ &nbsp;No text found. Use a text-based PDF.
                </div>""", unsafe_allow_html=True)
...
with c_right:
    ...
    if btn and resume_text and resume_text.strip():
        ...
        <div class="meta-strip">
          <div class="ms-cell">
            <div class="ms-k">Words</div>
            <div class="ms-v">{wc:,}</div>
          </div>
          <div class="ms-cell">
            <div class="ms-k">Tokens</div>
            <div class="ms-v">~{tc:,}</div>
          </div>
          <div class="ms-cell">
            <div class="ms-k">Top Score</div>
            <div class="ms-v">{top_conf}%</div>
          </div>
          <div class="ms-cell">
            <div class="ms-k">Status</div>
            <div class="ms-v ms-ok">✓ Done</div>
          </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown("""
        <div class="ib ib-err">
          ✕ &nbsp;Please paste resume text or upload a PDF.
        </div>""", unsafe_allow_html=True)
